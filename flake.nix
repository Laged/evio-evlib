      ...
      in
      {
        devShells.default = pkgs.mkShell {
          ...
          shellHook = ''
            ...
            echo "  download-datasets           : Download from Google Drive (~1.4 GB) if zip is absent"
            echo ""
            ...
            echo ""
            echo "🚀 Run the UI / Demos (after data prep):"
            echo "  run-mvp-demo           : Menu UI with detectors/datasets (OpenCV)"
            echo "  run-fan-rpm-demo       : Fan RPM (evlib/Polars on HDF5 export)"
            echo "  run-drone-detector-demo: Drone propellers (evlib/Polars on HDF5 export)"
            echo "  run-fan-detector       : Fan demo (legacy loader)"
            echo "  run-drone-detector     : Drone demo (legacy loader)"
            echo ""
            echo "🎬 Playback / Reference:"
            ...
            echo "  NOTE: run-demo-fan-ev3 needs convert-legacy-dat-to-hdf5 first"
            echo ""
            echo "🧪 Testing:"
